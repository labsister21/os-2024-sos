 ...
 *
 * Note, when returning from interrupt handler with iret, esp must be pointing
 * to eip pushed before or in other words, CPURegister, int_number and
 * error_code should be pop-ed from stack.
 *
 ...
 * When used for interrupt handler, cpu.stack is kernel state before C function
 * called, not user stack when it get called. Check InterruptStack and
 * interprivilege interrupt for more detail.
 *
 * @param cpu        CPU state
 * @param int_number Interrupt vector value
 * @param int_stack  Hardware-defined (x86) stack state, note: will not access
...
// Send ACK to PIC - @param irq Interrupt request number destination, note:
// ACKED_IRQ = irq+PIC1_OFFSET
 ...
 *
 * If inter-privilege interrupt raised, SS and ESP is automatically out of
 * main_interrupt_handler() parameter. Can be checked with ((int*) info) + 4 for
 * user $esp, 5 for user $ss
 *
 * Again, this function is not for normal function call, all parameter will be
 * automatically set when interrupt is called.
...
extern struct InterruptFrame *current_interrupt_frame;
extern bool syscall_return_value_flag;
