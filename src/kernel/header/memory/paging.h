...
// Used for memory manager, invalid physical page frame value for higher half
// kernel
 ...
 * @param lower_address   10-bit page frame lower address, note directly
 * correspond with 4 MiB memory (= 0x40 0000 = 1 Note:
 ...
 * Note: This data structure is volatile (can be modified from outside this
 * code, check "C volatile keyword"). MMU operation, TLB hit & miss also
 * affecting this data structure (dirty, accessed bit, etc).
 *
 * Warning: Address must be aligned in 4 KB (listed on Intel Manual), use
 * __attribute__((aligned(0x1000))), unaligned definition of PageDirectory will
 * cause triple fault
 *
 * @param table Fixed-width array of PageDirectoryEntry with size
 * PAGE_ENTRY_COUNT
 */
