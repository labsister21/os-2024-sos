...
extern uint32_t _linker_kernel_break_address;

/**
 * Load GDT from gdtr and complete init for protected mode. This procedure
 * implemented in asm. Note: This procedure will not activate CPU interrupt flag
 *
 * @param gdtr Pointer to already defined & initialized GDTR
 * @warning Invalid address / definition of GDT will cause bootloop after
...
/**
 *
 * Execute user program from kernel, one way jump. This function is defined in
 * asm source code.
 *
 * @param virtual_addr	Pointer into user program that already in memory
 * @warning							Assumming pointed memory
 * is properly loaded with instruction
 */
