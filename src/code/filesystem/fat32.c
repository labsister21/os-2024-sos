...
static void create_empty_directory_table(
...
static int8_t get_dir_table_from_cluster(uint32_t cluster, struct FAT32DirectoryTable *dir_entry) {
	if (fat32_driver_state.fat_table.cluster_map[cluster] != FAT32_FAT_END_OF_FILE) return -1;
	read_clusters(dir_entry, cluster, 1);
	if (
			strcmp(dir_entry->table[0].name, ".") == 0 &&
...
int8_t read(struct FAT32DriverRequest *request) {
	...
	if (get_dir_table_from_cluster(request->parent_cluster_number, &dir_table) != 0) return -1;
	...
	for (int i = 0; i < MAX_DIR_TABLE_ENTRY; ++i) {
		...
		if (
			...
			if (dir_entry->filesize > request->buffer_size) return -1;
...
int8_t read_directory(struct FAT32DriverRequest *request) {
	...
	for (int i = 0; i < MAX_DIR_TABLE_ENTRY; ++i) {
		...
		if (
			...
			// Request buffer too small
			if (CLUSTER_SIZE > request->buffer_size) return -1;
...
int8_t write(struct FAT32DriverRequest *request) {
	...
	struct FAT32DirectoryTable dir_table;
	// Parent folder not valid
	if (get_dir_table_from_cluster(request->parent_cluster_number, &dir_table) != 0) return 2;
	...
	if (free_cluster < needed_cluster) return -1;
	...
	for (int i = 0; i < needed_cluster; ++i) {
		...
		if (i + 1 == needed_cluster) {
			...
			if (remainder != 0) {
				...
				for (int i = remainder; i < CLUSTER_SIZE; ++i) buf.buf[i] = 0x0;
				src = (void *)&buf;
			}
		}
...
int8_t delete(struct FAT32DriverRequest *request) {
	...
	if (aFile) {
		...
		for (int i = 0; i < MAX_DIR_TABLE_ENTRY; ++i) {
			if (dir_table.table[i].user_attribute != UATTR_NOT_EMPTY) return 2;
		}
